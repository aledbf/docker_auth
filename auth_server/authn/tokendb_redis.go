...
func NewRedisTokenDB(url string, encryptKey string) (TokenDB, error) {
	client := redis.NewClient(&redis.Options{Addr: url})
	return &redisTokenDB{client, encryptKey}, nil
}

// NewRedisClusterTokenDB returns a new TokenDB structure which uses Redis Cluster as the storage backend.
func NewRedisClusterTokenDB(urls []string, encryptKey string) (TokenDB, error) {
	client := redis.NewClusterClient(&redis.ClusterOptions{Addrs: urls})
	return &redisTokenDB{client, encryptKey}, nil
}

// NewRedisSentinelTokenDB returns a new TokenDB structure which uses Redis Sentinel as the storage backend.
func NewRedisSentinelTokenDB(urls []string, masterName, encryptKey string) (TokenDB, error) {
...
func (db *redisTokenDB) GetValue(user string) (*TokenDBValue, error) {
	...
	if db.encryptKey != "" {
		b, err := db.Decrypt([]byte(result), []byte(db.encryptKey))
		if err != nil {
...
func (db *redisTokenDB) StoreToken(user string, v *TokenDBValue, updatePassword bool) (dp string, err error) {
	...
	if db.encryptKey != "" {
		data, err = db.Encrypt(data, []byte(db.encryptKey))
		if err != nil {
